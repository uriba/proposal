...
commonBegining (abs,fl) = (t_abs,t_fl)
    where
        ...
        t_abs = V.take (window_size + truncated_init) . V.dropWhile ((< start_time) . fst) $ abs
        t_fl = V.take (window_size + truncated_init) . V.dropWhile ((< start_time) . fst) $ fl
...
calcGrEl (abs,fl) = do
    ...
    if V.length t_abs < window_size + truncated_init then Nothing else Just ()
    if V.length t_fl < window_size + truncated_init then Nothing else Just ()
    abs_approx <- modCalcExpFit linearRegression . V.map (\(x,y) -> (realToFrac x,y)) $ V.drop truncated_init t_abs
    fl_approx <- modCalcExpFit linearRegression . V.map (\(x,y) -> (realToFrac x,y)) $ V.drop truncated_init t_fl
...
generateStrainData row plates = do
    ...
    return $ zipWith zip all_abs all_fl
...
main = do
    ...
    let normalized_od_fl = map (map (map (normalizeMod *** normalizeMod))) raw_od_fl_with_blanks
    ...
    let elgr_data = map (map (map calcGrEl)) . map (map (catMaybes . map pairMaybe)) $ normalized_od_fl
    let elgr_indexed_data = zip [1..] . map (map catMaybes) $ elgr_data
    mapM (\(x,y) -> writeFile (file_name_prefix ++ (show x) ++ "1.csv") $ printCSV (concatMap processedToPA y)) elgr_indexed_data
    mapM (\(x,y) -> writeFile (file_name_prefix ++ (show x) ++ "2.csv") $ printCSV (concatMap processedToPL y)) elgr_indexed_data

    return ()

processedToPA y = [map (show . (*3600) . fst) $ y,map (show . fst . snd) $ y]
processedToPL y = [map (show . (*3600) . fst) $ y,map (show . snd . snd) $ y]
...
pe (abs_a,abs_b) (fl_a,fl_b) start length = Just $ (fl (start + length/2))/(abs (start+length/2))
    where
        abs t = 2 ** (abs_a + (realToFrac t)*abs_b)
        fl t = 2 ** (fl_a + (realToFrac t)*fl_b)
