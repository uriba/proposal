...
The strain grown had a plasmid (pTAC) containing a fluorescent reporter gene \hl{one of YFP,mCherry}.
At any time-point we calculated the growth rate, the protein accumulation rate (that serves as a proxy for the promoter activity) and the protein level.
...
This initial result thus indicates the ability of our experimental system and analysis to quantify the relevant effects and shows one of the predictions of our model at play.
\begin{figure}[h]
...
Growth rate, protein level and protein accumulation rate values where calculated by averaging over 18 measurement point windows spanning 2.5 hours.
(A) Raw measurements of fluorescent levels and OD over time shows correlation of fluorescence with growth.
\hl{I've included both YFP and mCherry here as I assumed you'd like to see both. In the final version I thought to present only YFP as it looks somewhat better in panel (E).}
...
(D) Dividing the protein level by the growth rate at every point reveals a constant (normalized) level for growth rates in the range (0.45,0.65), following the theoretical prediction.
(E) Dividing the protein accumulation rate by the growth rate squared reveals a constant (normalized) level for growth rates in the range (0.45,0.65), following the theoretical prediction.
}
\label{time-gr-fig}
\end{figure}
