...
\label{growth-rate}
The ability to measure various properties of the target microorganisms when at balanced growth requires the ability to put them in this condition.
Our current experimental setup uses 96-well plates in which cultures are grown and therefore requires the ability to identify the stage of balanced growth and quantify the growth rate during this stage.

A naive approach will be to take OD (optical density) measurements at constant time intervals, look at intervals including a few (5, for the sake of the argument) such measurements, take the logarithm of the measured values and perform a linear regression to find the slope of a line that best approximates this measurements.
The slope of the line will be the growth rate.

This method has a few issues that need to be addressed:
\begin{enumerate}
\item The background OD must be determined and subtracted as otherwise the data will be severely distorted when transformed to log scale.
\item Detector noise can affect the estimate significantly - especially at low OD values.
\item Outliers (that can be caused due to occasional air-bubbles, scratches or misplacements of the plate in the reader etc.) can potentially affect the estimated growth rate.
\item Under different conditions the growth during the time period measured can vary substantially and may therefore create a systematic error depending on the growth rate.
\item Changes in medium composition due to the growth and initial state of the organisms (that are typically transferred from overnight cultures in stationary phase, potentially from different media than the one the are transferred to) result in non-uniform growth that typically goes through several stages, requiring a method to identify and verify the existence of a balanced growth stage.
\end{enumerate}
In order to overcome these issues and allow for large automatic assays we have introduced the following algorithm and experimental setup:
\begin{enumerate}
\item Each plate includes wells that contain just the growth media (hereafter referred to as “blank” wells).
Mean OD measurements of these wells are used to determine the background OD level.
\item Calculation of the standard deviation of measurements of the blank wells is used to estimate the detector noise.
\item The background is subtracted from each OD measurement of wells in the plate and if the resulting value is less than two times the standard deviation - then the value is being rounded up to this number (two times the standard deviation).
This procedure filters out very low values that can potentially cause diversion of solutions.
\item Initial dilution from an overnight culture is at least 1:2500.
Assuming stationary OD is about 0.5, this dilution results in an initial OD of approx. 0.0002 which is about 16 times lower than the typical detection noise in our system.
This high dilution ensures that once a signal is detected, the culture is already at balanced exponential growth.
For different types of microorganisms, media or detectors, the rule of thumb is to use dilution that will result in 4-7 generations before the culture first crosses the detection threshold.
\item Relevant period for analysis is calculated by dropping all measurements following the one with the highest measured value and all measurements preceding the last one with minimal measured value.
This “trimming” procedure removes artifacts caused due to vapor accumulation and initial noise (usually due to pipetting) as well as irrelevant effects occurring at stationary phase.
\item For each 6 consecutive measurements the logarithm of the measured values is taken and a linear fit is performed.
In order to reduce the effect of outlier measurements the fit uses trimmed-least-squares and allows up to two outlier measurements to be discarded.
the result is thus the optimal linear approximation of four points out of the original six-point sample.
\item As a safety measure - the same fit strategy is used for the expected two generations following initial detection (two generations imply the growth of the OD by 4-fold, which is easy to determine).
The two calculated growth rates are compared and allowed to vary by less than 5%.
\end{enumerate}
Preliminary results show this method to give relatively stable results of growth rate \hl{(data and figures needed)} as can be seen in the following graph.
The time of balanced growth is selected as the pre-defined number of generations, \hl{g=2?}, following initial detection.
The verification that the growth is indeed balanced during this phase is done by verifying no correlation between a sliding subwindow throughout this time and the time itself.
